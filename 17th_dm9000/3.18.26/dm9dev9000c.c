...
static u16 dm9000c_phy_read(board_info_t *, int);
static void dm9000c_phy_write(board_info_t *, int, u16);
...
static const struct net_device_ops dm9000c_netdev_ops = {
	.ndo_open		= dmfe_open,
	.ndo_stop		= dmfe_stop,
...
int __init dmfe_probe1(struct net_device *dev)
{
	...
	do {
		...
		if (id_val == DM9KS_ID || id_val == DM9010_ID) {
			...
			dev->netdev_ops	= &dm9000c_netdev_ops;		
...
#ifdef FLOW_CONTROL
	dm9000c_phy_write(db, 4, phy_reg4|(1<<10));
#else
	dm9000c_phy_write(db, 4, phy_reg4);
#endif //end of FLOW_CONTROL
	dm9000c_phy_write(db, 0, phy_reg0|0x200);
#else
	/* Fiber mode */
	dm9000c_phy_write(db, 16, 0x4014);
	dm9000c_phy_write(db, 0, 0x2100);
#endif //end of DM8606

	if (db->chip_revision == 0x1A)
	{
		//set 10M TX idle =65mA (TX 100% utility is 160mA)
		dm9000c_phy_write(db,20, dm9000c_phy_read(db,20)|(1<<11)|(1<<10));
		
		//:fix harmonic
		//For short code:
		//PHY_REG 27 (1Bh) <- 0000h
		dm9000c_phy_write(db, 27, 0x0000);
		//PHY_REG 27 (1Bh) <- AA00h
		dm9000c_phy_write(db, 27, 0xaa00);

		//PHY_REG 27 (1Bh) <- 0017h
		dm9000c_phy_write(db, 27, 0x0017);
		//PHY_REG 27 (1Bh) <- AA17h
		dm9000c_phy_write(db, 27, 0xaa17);

		//PHY_REG 27 (1Bh) <- 002Fh
		dm9000c_phy_write(db, 27, 0x002f);
		//PHY_REG 27 (1Bh) <- AA2Fh
		dm9000c_phy_write(db, 27, 0xaa2f);
		
		//PHY_REG 27 (1Bh) <- 0037h
		dm9000c_phy_write(db, 27, 0x0037);
		//PHY_REG 27 (1Bh) <- AA37h
		dm9000c_phy_write(db, 27, 0xaa37);
		
		//PHY_REG 27 (1Bh) <- 0040h
		dm9000c_phy_write(db, 27, 0x0040);
		//PHY_REG 27 (1Bh) <- AA40h
		dm9000c_phy_write(db, 27, 0xaa40);
		
		//For long code:
		//PHY_REG 27 (1Bh) <- 0050h
		dm9000c_phy_write(db, 27, 0x0050);
		//PHY_REG 27 (1Bh) <- AA50h
		dm9000c_phy_write(db, 27, 0xaa50);
		
		//PHY_REG 27 (1Bh) <- 006Bh
		dm9000c_phy_write(db, 27, 0x006b);
		//PHY_REG 27 (1Bh) <- AA6Bh
		dm9000c_phy_write(db, 27, 0xaa6b);
		
		//PHY_REG 27 (1Bh) <- 007Dh
		dm9000c_phy_write(db, 27, 0x007d);
		//PHY_REG 27 (1Bh) <- AA7Dh
		dm9000c_phy_write(db, 27, 0xaa7d);
		
		//PHY_REG 27 (1Bh) <- 008Dh
		dm9000c_phy_write(db, 27, 0x008d);
		//PHY_REG 27 (1Bh) <- AA8Dh
		dm9000c_phy_write(db, 27, 0xaa8d);
		
		//PHY_REG 27 (1Bh) <- 009Ch
		dm9000c_phy_write(db, 27, 0x009c);
		//PHY_REG 27 (1Bh) <- AA9Ch
		dm9000c_phy_write(db, 27, 0xaa9c);
		
		//PHY_REG 27 (1Bh) <- 00A3h
		dm9000c_phy_write(db, 27, 0x00a3);
		//PHY_REG 27 (1Bh) <- AAA3h
		dm9000c_phy_write(db, 27, 0xaaa3);
		
		//PHY_REG 27 (1Bh) <- 00B1h
		dm9000c_phy_write(db, 27, 0x00b1);
		//PHY_REG 27 (1Bh) <- AAB1h
		dm9000c_phy_write(db, 27, 0xaab1);
		
		//PHY_REG 27 (1Bh) <- 00C0h
		dm9000c_phy_write(db, 27, 0x00c0);
		//PHY_REG 27 (1Bh) <- AAC0h
		dm9000c_phy_write(db, 27, 0xaac0);
		
		//PHY_REG 27 (1Bh) <- 00D2h
		dm9000c_phy_write(db, 27, 0x00d2);
		//PHY_REG 27 (1Bh) <- AAD2h
		dm9000c_phy_write(db, 27, 0xaad2);
		
		//PHY_REG 27 (1Bh) <- 00E0h
		dm9000c_phy_write(db, 27, 0x00e0);
		//PHY_REG 27 (1Bh) <- AAE0h
		dm9000c_phy_write(db, 27, 0xaae0);
		//PHY_REG 27 (1Bh) <- 0000h
		dm9000c_phy_write(db, 27, 0x0000);
	}
}
...
static int dmfe_stop(struct net_device *dev)
{
	...
	/* RESET devie */
	dm9000c_phy_write(db, 0x00, 0x8000);	/* PHY RESET */
...
#endif
{
	...
	if (int_status & DM9KS_LINK_INTR) 
	{
		...
		{
			dm9000c_phy_read(db,0x1);
			if(dm9000c_phy_read(db,0x1) & 0x4) /*Link OK*/
			{
				/* wait for detected Speed */
				for(i=0; i<200;i++)
					udelay(1000);
				/* set media speed */
				if(dm9000c_phy_read(db,0)&0x2000) db->Speed =100;
				else db->Speed =10;
				break;
			}
			udelay(1000);
...
*/
static u16 dm9000c_phy_read(board_info_t *db, int reg)
{
...
*/
static void dm9000c_phy_write(board_info_t *db, int reg, u16 value)
{
	/* Fill the phyxcer register into REG_0C */
	iow(db, DM9KS_EPAR, DM9KS_PHY | reg);
...
{
	volatile u32 *bwscon;   //0x48000000
	volatile u32 *bankcon4; //0x48000014
	/* 17th_dm9000 */
