...
static 	dev_t dev_id;
static int major;
static struct input_replay *input_replay;
static struct class *cls;
static int replay_w,replay_r;
static char *replay_buf;
static struct timer_list time_dev;
...
static int replay_get_line(char *line)
{
	...
	{
		/* 获得一行的数据 */
		if((replay_buf[replay_r] == '\r') || (replay_buf[replay_r] == '\n'))
			break;
		else
		{
...
static void replay_timer_func(unsigned long data)
{
	...
	static unsigned int type, val, code;
	...
	/* 因为当下面的while执行完一个时间的时候进行不到下一个时间就会跳出来，这时候应该上报
	 * 因为如果不处理这边的话就会跳过这个时间
	 */
	if(pre_time != 0)
	{
	...
	while(1)
	{
		...
		else
		{
			...
			{
				mod_timer(&time_dev, jiffies + (time - pre_time));
...
static int replay_ioctl (struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		{
			/* 开始回放 */
			time_dev.expires = jiffies + 1;
			add_timer(&time_dev);
			break;
		}
		case INPUT_TAG:
		{
			/* 因为当我们注册驱动的时候会有一些垃圾的上报信息，我们通过这条指令来处理这个分隔符 */
			copy_from_user(buf, (const char __user *)arg, 100);
			buf[99] = '\0';
			myprintk("%s\n", buf);
			break;
		}
...
static void input_replay_cdev_set(struct input_replay *dev, int index)
{
	dev_id = MKDEV(major, index);
	int err;

	cdev_init(&dev->cdev, &replay_fops);
	err = cdev_add(&dev->cdev, dev_id, 1);
	if(err)
		printk(KERN_NOTICE"Error adding replay_cdev%d", err, index);
}
...
static int s3c_ts_init(void)
{
	...
	if(!replay_buf)
	{
		printk("can not alloc replay_buf\n");
		return -EIO;
	}
	/* 1. 分配一个input_dev结构体 */
	...
	input_replay = kzalloc(sizeof(struct input_replay), GFP_KERNEL);
	if(!input_replay)
	{
		printk("alloc input_replay error\n");
		goto kzalloc_failed;
	}
	input_replay_cdev_set(input_replay, 0);
	
	cls = class_create(THIS_MODULE, "input_replay");
	class_device_create(cls, NULL, MKDEV(major, 0), NULL, "input_replay");

	init_timer(&time_dev);
	time_dev.function = replay_timer_func;
